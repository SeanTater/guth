...
fn init_conv_state<B: Backend>(
    ...
    state.history = Some(Tensor::zeros([batch_size, in_channels, history_len], device));
    state
}
...
fn init_conv_transpose_state<B: Backend>(
    ...
    state.history = Some(Tensor::zeros([batch_size, out_channels, history_len], device));
    state
}
...
impl<B: Backend> ConvDownsample1d<B> {
    ...
    pub fn forward(&self, input: Tensor<B, 3>, state: &mut ConvDownsample1dState<B>) -> Tensor<B, 3> {
...
impl<B: Backend> ConvTrUpsample1d<B> {
    ...
    pub fn forward(&self, input: Tensor<B, 3>, state: &mut ConvTrUpsample1dState<B>) -> anyhow::Result<Tensor<B, 3>> {
        self.conv.forward(&mut state.conv, input)
    }
}
