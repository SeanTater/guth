use crate::modules::streaming_conv::{StreamingConv1dOp, StreamingConvState, StreamingConvTranspose1dOp};
...
impl<B: Backend> SeanetResnetBlock<B> {
    ...
    pub fn forward(&self, input: Tensor<B, 3>, state: &mut SeanetResnetBlockState<B>) -> Tensor<B, 3> {
...
impl<B: Backend> SeanetEncoder<B> {
    ...
    pub fn forward(&self, mut input: Tensor<B, 3>, state: &mut SeanetState<B>) -> Result<Tensor<B, 3>> {
...
fn init_layer_states<B: Backend>(
    ...
    layers
        ...
        .map(|layer| match layer {
            ...
            SeanetLayer::ConvTranspose1d(conv) => SeanetLayerState::ConvTranspose1d(
                init_conv_transpose_state(conv, batch_size),
            ),
            SeanetLayer::ResBlock(block) => SeanetLayerState::ResBlock(block.init_state(batch_size)),
        })
        .collect()
}

fn init_conv_state<B: Backend>(conv: &StreamingConv1dOp<B>, batch_size: usize) -> StreamingConvState<B> {
...
fn init_conv_transpose_state<B: Backend>(
    ...
    let history_len = conv
        .config
        .kernel_size
        .saturating_sub(conv.config.stride);
    let mut state = StreamingConvState::default();
    if history_len == 0 {
        return state;
    }
...
mod tests {
    ...
    fn build_layers(fixture: &SeanetFixture, device: &NdArrayDevice) -> Vec<SeanetLayer<TestBackend>> {
        fixture
            .layers
            .iter()
            .map(|layer| match layer {
