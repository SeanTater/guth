...
impl<B: Backend> TimestepEmbedder<B> {
    pub fn new(config: TimestepEmbedderConfig, device: &B::Device) -> Self {
        ...
        let proj_in =
            LinearConfig::new(config.frequency_embedding_size, config.hidden_size).init::<B>(device);
...
impl<B: Backend> FinalLayer<B> {
    pub fn new(config: FinalLayerConfig, device: &B::Device) -> Self {
        let norm = LayerNormConfig::new(config.model_channels)
            ...
            .with_bias(false)
            .init::<B>(device);
        let linear = LinearConfig::new(config.model_channels, config.out_channels).init::<B>(device);
        let mod_linear = LinearConfig::new(config.model_channels, config.model_channels * 2).init::<B>(device);
        Self {
...
impl SimpleMlpAdaLnConfig {
    pub fn new(in_channels: usize, model_channels: usize, out_channels: usize, cond_channels: usize) -> Self {
        Self {
            in_channels,
            model_channels,
            out_channels,
...
impl<B: Backend> SimpleMlpAdaLn<B> {
    pub fn new(config: SimpleMlpAdaLnConfig, device: &B::Device) -> Self {
        ...
        let cond_embed = LinearConfig::new(config.cond_channels, config.model_channels).init::<B>(device);
        let input_proj = LinearConfig::new(config.in_channels, config.model_channels).init::<B>(device);

        let mut res_blocks = Vec::with_capacity(config.num_res_blocks);
        for _ in 0..config.num_res_blocks {
            res_blocks.push(ResBlock::new(ResBlockConfig::new(config.model_channels), device));
        }
...
mod tests {
    ...
    fn apply_norm(norm: &mut LayerNorm<TestBackend>, weights: &NormWeights, device: &NdArrayDevice) {
        let gamma = Tensor::from_data(TensorData::new(weights.weight.clone(), [weights.weight.len()]), device);
    ...
    fn flow_net_matches_fixture() {
        ...
        apply_norm(&mut model.final_layer.norm, &fixture.weights.final_layer.norm, &device);
        apply_linear(&mut model.final_layer.linear, &fixture.weights.final_layer.linear, &device);
        apply_linear(
