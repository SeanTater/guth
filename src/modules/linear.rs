//! Helpers for linear and normalization ops on 3D tensors.
//!
//! WORKAROUND(BURN): These utilities work around edge cases in Burn backends when sequence
//! length is zero.
...
/// principle we could just call `linear.forward(input)`. However, some Burn
/// backends (especially non-ndarray ones) have issues with zero-length
/// dimensions. We have observed a divide-by-zero in `burn-ndarray` 0.20.1 when
/// the sequence dimension is zero (e.g. shape `[batch, 0, dim]`).
///
/// WORKAROUND(BURN): This helper works around the bug by constructing an empty output tensor
...
pub fn apply_linear_3d<B: Backend>(linear: &Linear<B>, input: Tensor<B, 3>) -> Tensor<B, 3> {
    ...
    if seq == 0 {
        // WORKAROUND(BURN): construct empty output with correct shape
        let [_in_dim, out_dim] = linear.weight.shape().dims::<2>();
        let device = input.device();
        return Tensor::from_data(
            BurnTensorData::new(Vec::<f32>::new(), [batch, 0, out_dim]),
            &device,
        );
    }
...
/// principle we could just call `norm.forward(input)`. However, several Burn
/// backends have issues with zero-length dimensions in various operations.
///
/// WORKAROUND(BURN): This helper returns the input unchanged if any dimension is zero (nothing
