...
impl<B: Backend> MimiModel<B> {
    ...
    pub fn from_config(config: &MimiConfig, device: &B::Device) -> Self {
        ...
        if (encoder_frame_rate - config.frame_rate).abs() > f32::EPSILON {
            ...
            let up_weight =
                Tensor::<B, 3>::zeros([config.seanet.dimension as usize, 1, 2 * stride], device);
    ...
    pub fn encode_to_latent(&self, input: Tensor<B, 3>) -> Tensor<B, 3> {
        ...
        let mut outputs = self
            .encoder_transformer
            .forward(emb, &mut transformer_state);
    ...
    fn to_framerate(
        &self,
        input: Tensor<B, 3>,
        state: Option<&mut ConvDownsample1dState<B>>,
    ) -> Tensor<B, 3> {
    ...
    pub fn load_state_dict(
        ...
        for (name, tensor) in state {
            ...
            if let Some(rest) = name.strip_prefix("encoder_transformer.") {
                apply_projected_transformer_weight(
                    &mut self.encoder_transformer,
                    rest,
                    tensor,
                    device,
                )?;
...
fn conv1d<B: Backend>(
    ...
    StreamingConv1dOp::new(
        conv_config(kernel_size, stride, dilation, pad_mode),
        weight,
        Some(bias),
    )
}
...
fn tensor1_from_data<B: Backend>(
    ...
    let shape: [usize; 1] = tensor
        ...
        .clone()
        .try_into()
        .map_err(|_| anyhow::anyhow!("Expected 1D tensor, got shape {:?}", tensor.shape))?;
...
fn tensor3_from_data<B: Backend>(
    ...
    let shape: [usize; 3] = tensor
        ...
        .clone()
        .try_into()
        .map_err(|_| anyhow::anyhow!("Expected 3D tensor, got shape {:?}", tensor.shape))?;
    let values = tensor_f32(tensor)?;
    Ok(Tensor::from_data(
        BurnTensorData::new(values, shape),
        device,
